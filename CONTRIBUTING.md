...
The following is a set of guidelines for contributing to `eventpoints-backend`.
...
- This project has two main blocks: API itself (built with Javascript) and Scrapers (built with Python/Scrapy and R)
- Pull Request **must be** made to `dev` branch
...
* **Perform a [cursory search](https://github.com/OSWeekends/eventpoints-backend/issues)** to see if the problem has already been reported. If it has **and the issue is still open**, add a comment to the existing issue instead of opening a new one.
...
* **Are you running the web app in a virtual machine?** If so, which VM software are you using and which operating systems and versions are used for the host and the guest?
...
* May **include screenshots and animated GIFs** which help you demonstrate the steps or point out the suggestion.
...
* [Beginner issues][https://github.com/OSWeekends/eventpoints-backend/labels/good%20first%20issue] - issues which should require a few lines of code.
...
* Place requires in the following order:
    * First, 3rd party libraries
    * Then files (by default, using @)


### CSS Styleguide
* As Bootstrap it's installed in our project, you may not need a lot of css work. Please, check [this link](https://getbootstrap.com/docs/4.3/utilities/borders/) before adding styles.
It contains a lot of extra classes as border, positions, colors, colors, etc. 
* If some manual css styling is required, must be done using `stylus` in the `style` block of each component
