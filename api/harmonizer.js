...
module.exports = function(goblinDB, sources, debugMode) {
  ...
  fs.readdir('../scrapers/output', function(err, files) {
    if (err) {
      ...
      newEventos.forEach(event => formatEvent(event, sources));

      if (debugMode) {
        console.log(`************ Nuevos eventos: ${newEventos.length}`);
      }

      let arrayEventos = _.concat(newEventos, currentEventos);
      ...
      */
      ...
      if (debugMode) {
        console.log(
          `************ Tras eliminar eventos duplicados: ${arrayEventos.length}`
        );
      }
